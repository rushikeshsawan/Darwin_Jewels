...
(xhttp.onload = function () {
  ...
  Array.from(e).forEach(function (e) {
    apiKeyList.add({
      ...
      createBy: e.createBy,
      apikey:
        '<input type="text" class="form-control apikey-value user-select-all" readonly value="' +
        e.apikey +
        '">',
  ...
  document
    ...
    .addEventListener("show.bs.modal", function (e) {
      e.relatedTarget.classList.contains("edit-item-btn")
        ...
        : e.relatedTarget.classList.contains("create-btn")
        ? ((document.getElementById("exampleModalLabel").innerHTML =
            "Create API Key"),
          (document
            .getElementById("api-key-modal")
          ...
          (document.getElementById("add-btn").style.display = "none"))
        : e.relatedTarget.classList.contains("regenerate-api-btn") &&
          ((document.getElementById("exampleModalLabel").innerHTML =
            "Regenerate API"),
          (document
            .getElementById("api-key-modal")
...
refreshCallbacks(),
  ...
  document.querySelectorAll(".create-btn").forEach(function (e) {
    ...
    e.addEventListener("click", function (e) {
      ...
      Array.from(e).forEach(function (e) {
        ...
        t == itemId &&
          ...
          (idField.value = t),
          (apiKeyNameField.value = e._values.name),
          (apiKeyField.value = new DOMParser()
            .parseFromString(e._values.apikey, "text/html")
            .body.querySelector(".apikey-value").value));
      });
    });
  }),
    Array.from(document.querySelectorAll(".regenerate-api-btn")).forEach(
      function (e) {
        e.addEventListener("click", function (e) {
          e.target.closest("tr").children[1].innerText,
            (itemId = e.target.closest("tr").children[1].innerText);
          ...
          Array.from(e).forEach(function (e) {
            ...
            t == itemId &&
              ((document.getElementById("apikey-element").style.display =
                "block"),
              (idField.value = t),
              (apiKeyNameField.value = e._values.name),
          ...
          deleteid.body.firstElementChild.innerHTML == itemId &&
            document
              ...
              .addEventListener("click", function () {
                apiKeyList.remove("id", t.outerHTML),
...
document
  .getElementById("createApi-btn")
  .addEventListener("click", function (e) {
    var t;
    if (0 == apiKeyNameField.value.length)
      return (
        (t = document.getElementById("api-key-error-msg")).classList.remove(
          "d-none"
        ),
        setTimeout(
          () =>
        ...
        !(t.innerHTML = "Please enter api key name")
      );
    0 < apiKeyNameField.value.length &&
      ((document.getElementById("apikey-element").style.display = "block"),
  ...
  addBtn.addEventListener("click", function (e) {
    ...
    if (
      ...
      return !(t.innerHTML = "Please enter api key name");
    "" !== apiKeyNameField.value &&
      "" !== apiKeyField.value &&
      (apiKeyList.add({
        id:
        ...
        createBy: document.querySelector(
          "#page-header-user-dropdown .user-name-text"
        ).innerHTML,
        apikey:
          '<input type="text" class="form-control apikey-value" readonly value="' +
          apiKeyField.value +
          '">',
  ...
  editBtn.addEventListener("click", function (e) {
    ...
    Array.from(t).forEach(function (e) {
      (isid = new DOMParser().parseFromString(e._values.id, "text/html")).body
        .firstElementChild.innerHTML == itemId &&
        e.values({
          id:
            '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ' +
            idField.value +
            "</a>",
          name: apiKeyNameField.value,
          createBy: e._values.createBy,
          apikey:
            '<input type="text" class="form-control apikey-value" readonly value="' +
            apiKeyField.value +
            '">',
          status: e._values.status,
          create_date: e._values.create_date,
          expiry_date: e._values.expiry_date,
        }),
        document.getElementById("close-modal").click();
    }),
      Swal.fire({
...
var chart,
  barchartColors = getChartColorsArray("hours_spent_chart");
barchartColors &&
  ((options = {
    series: [{ name: "API Key", data: [7, 11, 15, 20, 18, 23, 20] }],
    chart: {
      toolbar: { show: !1 },
      height: 90,
      type: "bar",
