...
<div class="container">
    <div class="row ">
        ...
        <div class="col-2 mb-2" style="text-align: right;"><a href="exportorder" class=""><button type="button" class="btn btn-primary align-self-end">Export</button></a></div>
    </div>
                ...
                <tr>
                    <td scope="col" style="width: 10px;">
                        <div class="form-check">
                            <input class="form-check-input export-checkbox" type="checkbox" value="<?= $orderItem['order_id'] ?>">
                        </div>
                    </td>
...
<!-- <script>
 $(document).ready(function() {
    $('.update-status').on('click', function() {
        ...
        $.ajax({
            url: 'update_order_status', // Replace with your controller route for updating status
            type: 'POST',
            data: {
                orderId: orderId,
                newStatus: newStatus
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (newStatus === 'delivered' && response.emailSent) {
                        console.log('Email sent successfully');
                    } else if (newStatus === 'delivered' && !response.emailSent) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        exportSelectedBtn.addEventListener("click", function () {
            ...
            if (selectedOrderIds.length > 0) {
                // Send the selected order IDs to the server for exporting
                window.location.href = "<?php echo base_url('export-selected?order_ids='); ?>" + selectedOrderIds.join(",");
            } else {
